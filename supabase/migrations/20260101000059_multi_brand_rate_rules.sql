-- ============================================================
-- payment_rate_rules.card_brand pode conter várias bandeiras juntas numa
-- só regra (ex: "Visa, MasterCard"), quando a taxa é igual pra elas — uso
-- legítimo, não erro de digitação. A comparação exata (card_brand =
-- p_card_brand) usada em create_orcamento/update_orcamento/
-- simulate_card_fee não reconhecia isso: pedir "Visa" contra uma regra
-- "Visa, MasterCard" não batia e caía (ou falhava) no fallback genérico.
--
-- rate_rule_matches_brand() interpreta card_brand como uma lista separada
-- por vírgula e verifica se a bandeira pedida está nela (com trim, pra
-- espaços depois da vírgula não atrapalharem). As três funções de
-- resolução de taxa passam a usar essa checagem em vez da igualdade
-- direta. simulator_available_brands() também passa a listar cada
-- bandeira da lista separadamente, não a string inteira.
-- ============================================================

create or replace function public.rate_rule_matches_brand(p_rule_brand text, p_requested_brand text)
returns boolean
language sql
stable
as $$
  select p_requested_brand is not null and exists (
    select 1 from unnest(string_to_array(p_rule_brand, ',')) as b(brand)
    where trim(b.brand) = p_requested_brand
  );
$$;
...
as $$
  select distinct trim(b.brand)
  from public.payment_rate_rules r
  join public.payment_machines m on m.id = r.machine_id
  cross join lateral unnest(string_to_array(r.card_brand, ',')) as b(brand)
...
begin
  ...
  if p_payment_method in ('pix', 'dinheiro') then
    ...
    -- Bandeira específica (inclusive quando a regra lista várias
    -- bandeiras juntas, ex: "Visa, MasterCard") tem prioridade; só cai
    -- para "qualquer bandeira" (card_brand is null) como fallback.
    select * into v_rule
    ...
    where machine_id = p_machine_id
      ...
      and (
        (p_card_brand is not null and (public.rate_rule_matches_brand(card_brand, p_card_brand) or card_brand is null))
...
begin
  ...
  where r.machine_id = v_machine.id
    ...
    and (
      (p_card_brand is not null and (public.rate_rule_matches_brand(r.card_brand, p_card_brand) or r.card_brand is null))
