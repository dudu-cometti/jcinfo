...
function ImageSlot({
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const result = await uploadPreorderCampaignImage(campaignId, field, formData)
    ...
    <div>
      ...
      {currentUrl ? (
        <div className="group relative aspect-video w-full overflow-hidden rounded-lg border border-neutral-200 sm:w-64">
          ...
          <form action={() => removePreorderCampaignImage(campaignId, field)} className="absolute right-1 top-1">
            <button
              type="submit"
  ...
  return (
    <div className="space-y-4">
      <ImageSlot
        ...
        hint="Formato retrato/paisagem, preenche o lado da página. Recomendado 1200x1500px"
        currentUrl={imageUrl}
      />
      <ImageSlot
        campaignId={campaignId}
