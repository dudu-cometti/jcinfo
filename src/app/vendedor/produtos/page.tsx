...
export default async function VendedorProdutosPage({ searchParams }: PageProps<'/vendedor/produtos'>) {
  ...
  let query = supabase
    .from('products')
    .select('id, name, sku, brand, model, price, promo_price, stock, status, category:categories(name)')
    .order('name')
    .limit(100)
  ...
  return (
    <div className="space-y-4">
      ...
      <form className="flex gap-3">
        <Input name="q" defaultValue={q} placeholder="Buscar por nome, SKU, marca, modelo..." className="max-w-sm" />
              ...
              <Tr key={product.id}>
                <Td>
                  ...
                  <div className="text-xs text-neutral-400">
                    {[product.brand, product.model, product.sku].filter(Boolean).join(' · ')}
                  </div>
                </Td>
